...
- Do not add a custom backend for breach jobs in the MVP.
...
Before implementation, add these guardrails to UI copy, worker limits, and test cases. Keep backend endpoints out of scope until a future feature explicitly needs accounts, persistence, sharing, or server-side processing.

...
The approved exception is `POST /api/ai-rerank`, a narrow server-side Gemini proxy. It receives bounded local solver candidate summaries, calls Gemini through Vertex AI / Gemini Enterprise Agent Platform with a server-side service account, and returns language decision support for plaintext candidates. It can accept, mark ambiguous, or reject candidate promotion. It is not allowed to decrypt, brute force, persist artifacts, or remove local confidence caps. If Gemini is unavailable, the app must fall back to local scoring.

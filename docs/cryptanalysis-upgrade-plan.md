...
- AI evidence rerank uses a server-side Gemini proxy through Vertex AI / Gemini Enterprise Agent Platform. It can accept, mark ambiguous, or reject local plaintext candidates, but it must not run solvers, invent keys, or remove local confidence caps.
